...
Ext.onReady(function () {
    ...
    function findJobs(user) {
        ...
        userJobs.query('UserID', user.getId()).each(function (item) { return jobArr.push(jobs.getById(item.get('JobID'))); });
        return jobArr;
    }
    function getJobString(value, metadata, record, rowIndex, colIndex, store, view) {
        ...
        return findJobs(record).map(function (j) { return j.get('Name'); }).join(', ');
    }
